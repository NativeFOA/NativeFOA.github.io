...
function initSoundfield(){
  ...
  function beamPair(cx,cy,angle,color,label){ctx.save();ctx.translate(cx,cy);ctx.rotate(angle);[-1,1].forEach(side=>{const g=ctx.createLinearGradient(0,0,side*64,0);g.addColorStop(0,'rgba(255,255,255,0)');g.addColorStop(1,color);ctx.globalAlpha=.16;ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(side*35,0,42,13,0,0,Math.PI*2);ctx.fill();ctx.globalAlpha=.36;ctx.strokeStyle=color;ctx.stroke()});ctx.restore();ctx.globalAlpha=.74;ctx.fillStyle='#53666d';ctx.font='700 9px ui-monospace,monospace';ctx.fillText(label,cx-15,cy+3);ctx.globalAlpha=1}
  function channelBands(){const labels=['W','Y','Z','X'],colors=['#687b81',palette.blue,palette.orange,palette.green],left=Math.max(24,w*.035),right=Math.min(w-24,w*.965),base=h*.86;labels.forEach((label,row)=>{const y=base+row*11;ctx.beginPath();for(let i=0;i<=80;i++){const u=i/80,x=left+(right-left)*u,amp=2.2+row*.55,yy=y+Math.sin(u*Math.PI*(10+row*2)+row*.8)*amp+Math.sin(u*Math.PI*27)*.65;i?ctx.lineTo(x,yy):ctx.moveTo(x,yy)}ctx.strokeStyle=colors[row];ctx.globalAlpha=.2;ctx.lineWidth=1;ctx.stroke();ctx.globalAlpha=.68;ctx.fillStyle=colors[row];ctx.font='800 8px ui-monospace,monospace';ctx.fillText(label,left-14,y+3)});ctx.globalAlpha=1}
  function latentStrip(){const x0=w*.31,x1=w*.69,y=h*.105,count=17;ctx.globalAlpha=.18;ctx.strokeStyle=palette.violet;ctx.setLineDash([3,6]);ctx.beginPath();ctx.moveTo(x0,y);ctx.lineTo(x1,y);ctx.stroke();ctx.setLineDash([]);for(let i=0;i<count;i++){const u=i/(count-1),x=x0+(x1-x0)*u,hh=5+((i*7)%13);ctx.fillStyle=[palette.blue,palette.violet,palette.green][i%3];ctx.globalAlpha=.16+(i%4)*.05;ctx.fillRect(x-1,y-hh/2,2,hh)}ctx.globalAlpha=.62;ctx.fillStyle='#53666d';ctx.font='700 8px ui-monospace,monospace';ctx.fillText('FOA LATENT · FRAME-ALIGNED CONDITIONING',x0,y-13);ctx.globalAlpha=1}
  function field(){ctx.save();const paths=[[.06,.7,.28,.18,.72,.82,.96,.34,palette.blue],[.03,.43,.28,.83,.73,.12,.98,.57,palette.violet],[.11,.2,.38,.65,.64,.35,.91,.8,palette.green]];paths.forEach((p,i)=>{const g=ctx.createLinearGradient(w*p[0],0,w*p[6],0);g.addColorStop(0,'rgba(255,255,255,0)');g.addColorStop(.25,p[8]);g.addColorStop(.75,p[8]);g.addColorStop(1,'rgba(255,255,255,0)');ctx.strokeStyle=g;ctx.globalAlpha=.14;ctx.lineWidth=1.2;ctx.setLineDash([3+i*2,8+i*2]);ctx.beginPath();ctx.moveTo(w*p[0],h*p[1]);ctx.bezierCurveTo(w*p[2],h*p[3],w*p[4],h*p[5],w*p[6],h*p[7]);ctx.stroke()});ctx.setLineDash([]);motes.forEach((m,i)=>{const x=w*m.u,y=h*m.v,r=.7+m.d*1.8;ctx.globalAlpha=.1+m.d*.2;ctx.fillStyle=m.c;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();if(i%9===0){ctx.strokeStyle=m.c;ctx.globalAlpha=.08;ctx.beginPath();ctx.moveTo(x-r*7,y);ctx.lineTo(x+r*7,y);ctx.stroke()}});channelBands();latentStrip();beamPair(w*.255,h*.69,-.4,palette.blue,'W ± Y');beamPair(w*.745,h*.29,.4,palette.orange,'W ± Z');beamPair(w*.79,h*.83,-.15,palette.green,'W ± X');ctx.restore()}
  function draw(time){ctx.clearRect(0,0,w,h);field();const R=Math.min(w,h)*.18;sphere(w*.14,h*.3,R,.38,-.12,'','moveA',time);sphere(w*.86,h*.7,R,-.44,-.08,'','moveB',time);if(!matchMedia('(prefers-reduced-motion: reduce)').matches)requestAnimationFrame(draw)}
  addEventListener('resize',resize);resize();requestAnimationFrame(draw)
}
