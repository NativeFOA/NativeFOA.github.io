...
function initSoundfield(){
  ...
  function draw(time){ctx.clearRect(0,0,w,h);field();const R=Math.min(w,h)*.145;sphere(w*.135,h*.3,R,.38,-.12,'','moveA',time);sphere(w*.865,h*.255,R,-.32,-.08,'','remove',time);sphere(w*.84,h*.71,R,.48,-.1,'','replace',time);if(!matchMedia('(prefers-reduced-motion: reduce)').matches)requestAnimationFrame(draw)}
  addEventListener('resize',resize);resize();requestAnimationFrame(draw)
}
