...
function initSoundfield(){
  ...
  function field(time){ctx.save();const paths=[[.08,.72,.31,.12,.7,.88,.94,.3,palette.blue],[.03,.45,.27,.86,.72,.08,.98,.55,palette.violet],[.12,.18,.38,.64,.63,.36,.9,.82,palette.green]];paths.forEach((p,i)=>{const g=ctx.createLinearGradient(w*p[0],0,w*p[6],0);g.addColorStop(0,'rgba(255,255,255,0)');g.addColorStop(.25,p[8]);g.addColorStop(.75,p[8]);g.addColorStop(1,'rgba(255,255,255,0)');ctx.strokeStyle=g;ctx.globalAlpha=.13;ctx.lineWidth=1.2;ctx.setLineDash([3+i*2,8+i*2]);ctx.beginPath();ctx.moveTo(w*p[0],h*p[1]);ctx.bezierCurveTo(w*p[2],h*p[3],w*p[4],h*p[5],w*p[6],h*p[7]);ctx.stroke()});ctx.setLineDash([]);motes.forEach((m,i)=>{const u=(m.u+time*.000008*(.65+m.d))%1,x=w*u,y=h*(m.v+Math.sin(time*.00045+i)*.012*m.d),r=.7+m.d*1.8;ctx.globalAlpha=.1+m.d*.22;ctx.fillStyle=m.c;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();if(i%9===0){ctx.strokeStyle=m.c;ctx.globalAlpha=.08;ctx.beginPath();ctx.moveTo(x-r*7,y);ctx.lineTo(x+r*7,y);ctx.stroke()}});for(let i=0;i<7;i++){const u=(i/7+time*.000035)%1,x=w*(.08+.84*u),y=h*(.5+Math.sin(u*Math.PI*2+time*.00025)*.24);glow(x,y,8+(i%3)*3,[palette.blue,palette.violet,palette.green][i%3],.26)}ctx.restore()}
  function draw(time){ctx.clearRect(0,0,w,h);field(time);const yaw=time*.000075+mx*.16,pitch=-.13+my*.08,R=Math.min(w,h)*.18;sphere(w*.14,h*.3,R,yaw,pitch,'','moveA',time);sphere(w*.86,h*.7,R,-yaw*.78,pitch*.72,'','moveB',time);if(!matchMedia('(prefers-reduced-motion: reduce)').matches)requestAnimationFrame(draw)}
